...
public class SessionInfoBeanTest {
    ...
    @Test
    @WithMockUser(username = "user1", authorities = {"REGISTERED_USER"})
    public void testLoggedIn() {
        ...
        Assertions.assertEquals("REGISTERED_USER", sessionInfoBean.getCurrentUserRoles(), "sessionInfoBean.getCurrentUserRoles does not return authenticated user's roles");
        Assertions.assertTrue(sessionInfoBean.hasRole("REGISTERED_USER"), "sessionInfoBean.hasRole does not return true for a role the authenticated user has");
